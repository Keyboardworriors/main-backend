...
class DiaryAPIView(APIView):
    ...
    def get(self, request, diary_id=None):
        ...
        # 특정 일기 조회
            ...
            # 일기가 없으면 일기 쓰기 안내
            return Response({
                'message': 'No diary found for the selected date.',
                'data': None
            }, status=status.HTTP_200_OK)
        # 모든 일기 조회 ( 메인페이지 ) -> 쓴 날짜 목록 보내기
        diaries = Diary.objects.filter(member=request.user).values_list('created_at')
        diary_dates = [diary['created_at'] .date() for diary in diaries]

        return Response({
            'message': 'Successfully retrieved diary dates.',
            'data': diary_dates,
        },  status=status.HTTP_200_OK)
    ...
    def delete(self, request, diary_id=None):
        ...
        diary.delete()
        return Response({
            'message': 'Successfully deleted diary.'
        }, status=status.HTTP_200_OK)