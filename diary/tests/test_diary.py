...
class DiaryTests(APITestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_user(email=self.email, password=self.password,nickname=self.nickname)
    ...
    def test_create_diary(self):
        ...
        self.assertEqual(response.data["data"]["diary_title"], payload["diary_title"])
    ...
    def test_search_diary(self):
        """ 일기 검색 테스트 """
        payload = {"q": "test"}
        response = self.client.post("/api/diary/search/", data=payload, format="json")
