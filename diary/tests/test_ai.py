...
class EmotionMusicAITests(APITestCase):
    def test_extract_emotions_from_diary(self):
        ...
        response = self.client.post("/api/diary/recommendation-keyword", data=diary_content, format="json")
    ...
    def test_recommend_music_based_on_emotions(self,):
        """감정 + 장르 기반 음악 추천 테스트"""
        data = {
            "moods": ["피곤", "초조"],
            "favorite_genre": "pop"
        }
        response = self.client.post("/api/diary/music/recommend/", data=data, format="json")
